...
typealias AnnouncementUseCase = AnnouncementListUseCase & AnnouncementDetailUseCase
...
final class DefaultAnnouncementUseCase: AnnouncementUseCase {
    ...
    func fetchAnnouncementDetail(_ id: Int) {
        ...
        if let foundAnnouncement = announcementList.first(where: { $0.id == id }) {
            announcement.accept(foundAnnouncement)
        }
    }
}
